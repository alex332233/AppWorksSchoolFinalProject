...
contract AlexUniswapV3toV2ArbitrageTryTRB {
    ...
    function uniswapV3SwapCallback(
        ...
        if (zeroForOne) {
            ...
            path = getPathForTokens(uniV3token1, uniV3token0); // borrow token1, so swap token1 to token0 at UniswapV2
        ...
        console.log(
            uniV3token0,
            "amount From UniV2Pair: ",
            amountFromUniV2Pair
        );
        console.log(
            uniV3token0,
            "amount repay to UniV3Pool: ",
            amountIntoUniV3Pool
        );
        ...
        if (amountFromUniV2Pair >= amountIntoUniV3Pool) {
            ...
            require(
                IERC20(uniV3token0).balanceOf(caller) >= loss,
                "Not enough token to repay"
            );
            ...
            console.log(
                "caller balance: ",
                IERC20(uniV3token0).balanceOf(caller)
            );
        }
    }
